...
class CheckSubscriptionsExpirationTest extends TestCase
{
    ...
    private function makeEntrepriseWithUser(string $plan, string $status, \Carbon\Carbon $expiresAt): array
    {
        ...
        $user = User::factory()->create([
            ...
            'email' => 'test-check-'.uniqid().'@primegest-test.dev',
        ]);
    ...
    public function test_trial_warning_not_sent_on_day_of_creation(): void
    {
        ...
        [, $user] = $this->makeEntrepriseWithUser('pro', 'trial', now()->addDays(2));

        $this->artisan('subscriptions:check')->assertExitCode(0);

        Mail::assertNotSent(SubscriptionExpiringSoon::class, fn ($mail) => $mail->hasTo($user->email));
    }
    ...
    public function test_trial_warning_sent_one_day_before_expiry(): void
    {
        ...
        [, $user] = $this->makeEntrepriseWithUser('pro', 'trial', now()->addHours(20));

        $this->artisan('subscriptions:check')->assertExitCode(0);

        Mail::assertSent(SubscriptionExpiringSoon::class, fn ($mail) => $mail->hasTo($user->email));
    }

    public function test_paid_warning_sent_two_days_before(): void
    {
        Mail::fake();

        // Abonnement payé expire dans 2 jours — fenêtre 7 jours, DOIT envoyer
        [, $user] = $this->makeEntrepriseWithUser('premium', 'confirmed', now()->addDays(2));
