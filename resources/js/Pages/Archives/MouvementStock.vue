<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h1 class="text-2xl font-bold">Archives des mouvements de stock</h1>
      ...
      <div v-for="archive in archiveDates" :key="archive.date" class="bg-white p-6 rounded shadow hover:shadow-lg transition">
        <div class="flex justify-between items-start mb-4">
          ...
          <div class="bg-blue-100 text-blue-800 px-3 py-1 rounded text-sm font-semibold">
            {{ archive.count }} mouvement{{ archive.count > 1 ? 's' : '' }}
          </div>
        </div>
    ...
    <div v-else class="bg-white p-6 rounded shadow text-center text-gray-400">
      ...
      Aucune archive de mouvement trouvée
    </div>
  </div>
</template>
...
function previewArchive(date) {
  router.get(`/archives/mouvements-stock/${date}/preview`)
}

function downloadArchive(date) {
  window.location.href = `/archives/mouvements-stock/${date}/download`
}

function printArchive(date) {
  window.location.href = `/archives/mouvements-stock/${date}/preview?print=true`
}
</script>
